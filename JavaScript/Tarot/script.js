...
let tirages = 0;
let cartesTirees = new Set()
const numeros = ["∞", "I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX", "X", "XI", "XII", "XIII", "XIV", "XV", "XVI", "XVII", "XVIII", "XIX", "XX", "XI"]
const listeCartes = [
    {
        nom: "Le mat",
...
function tirerCarte() {
    ...
    let rand
    do {
        rand = getRandomIntInclusive(0,21)
    } while (cartesTirees.has(rand))
    cartesTirees.add(rand)
    if (tirages == 3) {
        document.querySelector("#tirage").classList.add("hide")
        document.querySelector("#nouveautirage").classList.remove("hide")
    }
    let carte = document.createElement("div")
    let image = document.createElement("img")
    let nom = document.createElement("h2")
    let signification = document.createElement("p")
    carte.classList.add("carte")
    image.setAttribute("src",`images/${rand}.jpg`)
    nom.innerHTML = `${numeros[rand]} - ${listeCartes[rand]["nom"]}`
    signification.innerHTML = `${listeCartes[rand]["signification"]}`
    carte.appendChild(image)
    carte.appendChild(nom)
    carte.appendChild(signification)
    document.querySelector("#carteContainer").appendChild(carte)
}

function nouveauTirage() {
    tirages = 0
    document.querySelector("#tirage").classList.remove("hide")
    document.querySelector("#nouveautirage").classList.add("hide")
    document.querySelector("#carteContainer").innerHTML = ""
    cartesTirees.clear()
}